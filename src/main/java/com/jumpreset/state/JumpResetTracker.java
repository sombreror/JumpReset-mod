...
/**
 * JumpResetTracker — v2.0.0
 *
 * Changes from v1.9.0:
 *
 * New — SessionStats integration.
 *   dispatchResult() now also calls sessionStats.record() so every classified
 *   attempt (PERFECT / GOOD / LATE / TOO_EARLY) is counted in the session
 *   aggregate. SessionStats tracks total, per-class counts, current streak,
 *   best streak, and average score. Exposed as a public field so ConfigScreen
 *   and the debug overlay can read it without an accessor call.
 */
